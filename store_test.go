...
func (s *StoreSuite) TestCreateBird() {
	...
	res, err := s.db.Query(`SELECT COUNT(*) FROM birds WHERE description='test description' AND bird='test species'`)
	if err != nil {
		s.T().Fatal(err)
	}
...
func (s *StoreSuite) TestGetBird() {
	...
	_, err := s.db.Query(`INSERT INTO birds (bird, description) VALUES('bird','description')`)
	if err != nil {
		s.T().Fatal(err)
	}
